...
void profiler_print_times(GameState* gameState) {
    u32 microseconds[PROFILER_TIME_COUNT];
    char text_buffer[196];
    Input* input = &gameState->input[CS_CTRL_CONTROLLER_PORT];
    ...
    if (show_profiler) {
        ...
        u32 total_rsp = microseconds[PROFILER_TIME_RSP_GFX] + microseconds[PROFILER_TIME_RSP_AUDIO] + microseconds[PROFILER_TIME_RSP_OTHER];
        u32 max_rdp = MAX(MAX(microseconds[PROFILER_TIME_TMEM], microseconds[PROFILER_TIME_CMD]), microseconds[PROFILER_TIME_PIPE]);

        u32 frameDivisor = R_UPDATE_RATE == 2 ? 333 : 500;

        Color_RGBA32 col = { 255, 255, 255, 255 };
        GfxPrint_Print(gameState, 2, 1, col,
        // sprintf(text_buffer,
            "FPS:   %5.2f\n"
            "\n"
            "CPU:   %d (%d%%)\n"
            ...
            total_cpu, total_cpu / frameDivisor, 
            // microseconds[PROFILER_TIME_CONTROLLERS],
            microseconds[PROFILER_TIME_GRAPH],
            microseconds[PROFILER_TIME_LINK],
            // microseconds[PROFILER_TIME_ACTORS] - microseconds[PROFILER_TIME_LINK],
            // microseconds[PROFILER_TIME_GFX],
            microseconds[PROFILER_TIME_AUDIO] * R_UPDATE_RATE, // audio is 60Hz, so R_UPDATE_RATE * the average
            max_rdp, max_rdp / frameDivisor,
            microseconds[PROFILER_TIME_TMEM],
            microseconds[PROFILER_TIME_CMD],
            microseconds[PROFILER_TIME_PIPE],
            total_rsp, total_rsp / frameDivisor,
            microseconds[PROFILER_TIME_RSP_GFX],
            microseconds[PROFILER_TIME_RSP_AUDIO],
            microseconds[PROFILER_TIME_RSP_OTHER]
        );
        ...
        // GfxPrint_PrintString(&printer, text_buffer);
